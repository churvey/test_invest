...
def volume_up(data):
    ...
    # v_slide = sliding_window_view(v, slide)
    # candi = (data["vma_5"] <= 0.5) & (data["change"] < -0.05)
    # 
    # candi = (data["vma_5"] <= 0.4) & (np.abs(data["change"]) < 0.05)
    candi = low_pos & (data["vma_5"] <= 0.8) & (data["change"] > 0)
    # candi = (data["vma_5"] <= 0.5)
    
    # print("change", data["change"])
    
    # [101835 rows x 147 columns]
    ...
    print(f"{data['instrument'][0]} profile {profile[~np.isnan(profile)].max()} {profile[~np.isnan(profile)].min()} {np.nanmean(profile)} {np.nanquantile(profile, 0.99)} {np.nanquantile(profile, 0.5)} ")
    ...
    # no_upper_before_down = np.all(v_slide <= 4, axis = -1)
    # pad = np.full(len(open) - len(no_upper_before_down), False)
    # # print(f"down: {down.shape}, v_slide:{v_slide.shape}, no_upper:{no_upper_before_down.shape} {pad.shape}")
    # no_upper_before_down = np.concatenate(
    #    [pad , no_upper_before_down]
    # )
